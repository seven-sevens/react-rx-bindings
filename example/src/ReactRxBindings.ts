...
export function useReactRxBindings<T extends ReactRxBindingViewModel>(instClosure: () => T): ReactRxBindingViewModelState<T> {
    ...
    useEffect(() => {
        let subscriptions = (classesWithReactive[inst.constructor.name] ?? [])
            .map((propertyKey: string) => {
                // @ts-ignore
                return inst[propertyKey].pipe(distinctUntilChanged()).subscribe((value: any) => {
                    let keyName = reformatPropertyKeyName(propertyKey);
